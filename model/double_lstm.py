...
def double_lstm(drop,embedding_matrix,max_sentence_length,max_sentence_number):
    ...
    context_vector = TimeDistributed(Embedding(len(embedding_matrix), 300, input_length=(max_sentence_length,), mask_zero=False,
                               weights=[embedding_matrix], name='sentence_emd2'))(inputs)
    ...
    lstm1 = TimeDistributed(Bidirectional(GRU(256, return_sequences=False, dropout=drop)))(context_vector)


    max_pool = Attention(max_sentence_number)(lstm1)


    lstm2 = Bidirectional(GRU(256, return_sequences=False, dropout=drop))(lstm1)

    conc2 = concatenate([max_pool, lstm2])

    conc3 = Dense(64, activation="relu")(conc2)

    out = Dropout(drop)(conc3)
...
if __name__ == "__main__":
    ...
    for i in drop:
        ...
        model = double_lstm(i,embedding_matrix,100,58)
