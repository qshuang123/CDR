...
from keras.layers import Dense, Dropout, Bidirectional, GRU, Embedding, TimeDistributed, GlobalMaxPooling1D, \
    concatenate, Lambda, Dot, Permute, Concatenate, Multiply, Add, GlobalAveragePooling1D, Conv2D, MaxPooling2D, Conv1D
...
def get_context(input):
    ...
    context_rep = Conv2D(filters=[32, 32], kernel_size=[32, 32], padding='same', activation='relu')(context)
    print("contxt",context_rep.shape)
    return context_rep
...
if __name__ == "__main__":
    ...
    drop = [0.3]
    for drop_i in drop:
        logging.info('basemodel+position+entity_mask_ESIM33' + str(drop_i))
