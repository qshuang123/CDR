...
from keras.layers import Dense, Dropout, Bidirectional, GRU, Embedding, TimeDistributed, GlobalMaxPooling1D, \
    concatenate, Lambda, Dot, Permute, Concatenate, Multiply, Add, Conv1D, MaxPooling1D, Flatten, Conv2D
...
def submult(input_1, input_2):
    ...
    res = Lambda(lambda x: K.abs(x), output_shape=unchanged_shape)(sub)
    return res
...
def basemodel_position(drop,embedding_matrix,max_sentence_length,max_sentence_number):
    ...
    print(lstm.shape)
    c = Lambda(get_context, output_shape=get_context_output_shape)(lstm)
    context_rep = Capsule(num_capsule=16, dim_capsule=32, routings=10, share_weights=True)(c)
    context_rep = Flatten()(context_rep)
...
if __name__ == "__main__":
    ...
    logging.info('basemodel+position+entity_mask_ESIM8_')
