...
def get_context(input):
    """Align text representation with neural soft attention"""
    context = Lambda(slice, arguments={'h1': 199, 'h2': 200})(input)
    context = keras.layers.Reshape([23, 512])(context)
    atten = Dense(1)(context)
    atten = keras.layers.Reshape([23])(atten)
    atten = Lambda(lambda x: softmax(x, axis=1),
                     output_shape=unchanged_shape)(atten)
    context_rep = Lambda(lambda x: K.batch_dot(x[0], x[1]),
                     output_shape=unchanged_shape)([atten, context])
    print("context_rep", context_rep.shape)
    return context_rep
...
if __name__ == "__main__":
    ...
    drop = [0.3]
    for drop_i in drop:
        logging.info('basemodel+position+entity_mask_ESIM8' + str(drop_i))
