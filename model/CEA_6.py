...
def entity_mask_atten(input):
    entity1_mask = input[0]
    entity1_vec = K.sum(entity1_mask, axis=-1)
    sum = K.sum(entity1_vec, axis=-1)
    weighted = entity1_vec/sum
    entity1_rep = input[1]
    res = Dot(axes=1)([weighted, entity1_rep])
    return res


def entity_mask_atten_output_shape(input_shape):
...
def basemodel_position(drop,embedding_matrix,max_sentence_length,max_sentence_number):
    ...
    entity1_out = Lambda(entity_mask_atten, output_shape=entity_mask_atten_output_shape)([entity1_mask, entity1_atten])
    entity2_out = Lambda(entity_mask_atten, output_shape=entity_mask_atten_output_shape)([entity2_mask, entity2_atten])

    res = concatenate([context_rep, entity1_out, entity2_out])
    res = Dense(64)(res)
...
if __name__ == "__main__":
    ...
    logging.info('basemodel+position+entity_mask_ESIM33')
