...
def basemodel_position(drop,embedding_matrix,max_sentence_length,max_sentence_number,k):
    ...
    context_vector = TimeDistributed(Embedding(len(embedding_matrix), 300, input_length=(max_sentence_length,), mask_zero=False,
                               weights=[embedding_matrix], name='sentence_emd', trainable=False))(inputs)
    pos_embedding=Embedding(5801, 30, input_length=(max_sentence_length,), mask_zero=False,name='pos_emd',trainable=False)
    ...
    # entity_rep = Add()([entity1_out, entity2_out])

    # context_rep = Lambda(get_context, output_shape=get_context_output_shape)([lstm, entity_rep])
...
def run(model,x_train,y_train,x_develop,y_develop,x_test,y_test,train_pos1,dev_pos1,test_pos1,train_pos2,dev_pos2,test_pos2,
    ...
    m2=Model(model.input,model.get_layer('dot_1').output)

    pred_test_y = m2.predict([x_test, test_pos1, test_pos2, test_entity1_pos, test_entity2_pos],
                            batch_size=batch_size, verbose=0)
    # p, r, f = f1(y_test, (pred_test_y > 0.5).astype(int))
    # print("-    Val F1 Score: {:.4f}".format(f))
    print(pred_test_y[0])
    save_file('entity_align',pred_test_y)
...
if __name__ == "__main__":
    ...
    logging.info('basemodel+position+entity_mask_ESIM11_top_100')
    # for k in top_k:
    model = basemodel_position(0.3, embedding_matrix,200,23,400)
