...
def get_context(input):
    ...
    atten_topk, topk_pos = K.tf.nn.top_k(atten, 400)
...
def basemodel_position(drop,embedding_matrix,max_sentence_length,max_sentence_number):
    ...
    context_rep = Lambda(get_all_context, output_shape=get_all_context_output_shape)(lstm)
    context1_rep = Lambda(get_context, output_shape=get_context_output_shape)([lstm, entity1_out])
    context2_rep = Lambda(get_context, output_shape=get_context_output_shape)([lstm, entity2_out])

    res = concatenate([context_rep, context1_rep, context2_rep, entity1_out, entity2_out])
    res = Dense(64)(res)
...
if __name__ == "__main__":
    ...
    logging.info('basemodel+position+entity_mask_ESIM13_top_100')
