...
def basemodel_position(drop,embedding_matrix,max_sentence_length,max_sentence_number):
    ...
    context_vector = TimeDistributed(Embedding(len(embedding_matrix), 200, input_length=(max_sentence_length,), mask_zero=False,
...
def run(model,x_train,y_train,x_develop,y_develop,x_test,y_test,train_pos1,dev_pos1,test_pos1,train_pos2,dev_pos2,test_pos2,
    ...
    for i in range(25):
        ...
        if f > 0.6:
            f = open('../result/' + str(f)+"_predict", 'wb')
            pickle.dump(pred_test_y, f)
            f.close()
            model.save('../result/' + 'my_model'+str(f)+'.h5')
...
if __name__ == "__main__":
    ...
    drop = [0.3]
    for drop_i in drop:
        logging.info('basemodel+position+entity_mask_ESIM3' + str(drop_i))
