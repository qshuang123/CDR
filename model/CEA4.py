...
def attention(input):
    attention = Dense(1)(input)
    attention = keras.layers.Reshape([23])(attention)
    weighted = Lambda(lambda x: softmax(x, axis=1), output_shape=unchanged_shape)(attention)
    res = Dot(axes=1)([weighted, input])
    print(res.shape)
    return res
...
def basemodel_position(drop,embedding_matrix,max_sentence_length,max_sentence_number):
    ...
    relation_rep = concatenate([context_rep, entity1_atten, entity2_atten])
    relation = Bidirectional(GRU(256, return_sequences=True, dropout=drop))(relation_rep)
    relation = GlobalMaxPooling1D()(relation)

    res = Dense(64)(relation)
...
if __name__ == "__main__":
    ...
    logging.info('basemodel+position+entity_mask_ESIM4')
